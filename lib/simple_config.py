...
SYSTEM_CONFIG_PATH = "/etc/electrum-reden.conf"
...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrum-reden directory", path)
        return path
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-reden.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-reden.conf into user_config[]."""
    if not path:
        return {}
