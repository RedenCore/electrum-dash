...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-REDEN"
  !define PRODUCT_WEB_SITE "https://github.com/akhavr/electrum-reden"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-reden-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-reden.ico"
...
Section
  ...
  File /r "dist\electrum-reden\*.*"
  File "icons\electrum-reden.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-reden-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-reden-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-reden-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-reden-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-reden-${PRODUCT_VERSION}.exe" 0


  ;Links reden: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\reden" "" "URL:reden Protocol"
  WriteRegStr HKCU "Software\Classes\reden" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\reden" "DefaultIcon" "$\"$INSTDIR\electrum-reden.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\reden\shell\open\command" "" "$\"$INSTDIR\electrum-reden-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-reden.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\reden"
