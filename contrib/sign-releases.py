...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.reden.org -keyalg RSA -keysize 2048 \
        -validity 10000
...
Manual signing:
    ...
    jarsigner -verbose \
        ...
        -sigfile reden-electrum \
        -keystore ~/.jks/keystore \
        Electrum_REDEN-3.0.6.1-release-unsigned.apk \
        electrum.reden.org
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_REDEN-3.0.6.1-release-unsigned.apk \
        Electrum_REDEN-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^Electrum-REDEN-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Electrum-REDEN-{version}'
PPA_SOURCE_NAME = 'electrum-reden'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_REDEN-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Electrum_REDEN-{version}-release.apk'
